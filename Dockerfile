...
# Variables NEXT_PUBLIC_* deben existir EN BUILD-TIME: Next las hornea en el bundle del
# cliente durante `pnpm build`. Railway inyecta las del servicio como build args; sin
# declararlas con ARG+ENV aquí, quedarían `undefined` en el JS del navegador (p.ej. el
# botón de login de Firebase no aparecería). Estas son config PÚBLICA (no secretos).
